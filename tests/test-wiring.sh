...
# seven (event, matcher, command) triples, byte for byte. Claude runs an
...
# six to seven with #91's PermissionRequest entry.
install_claude
...
assert_eq "$(printf '%s\n' "$got" | grep -c .)" "7" "the generated file carries all seven roost hook entries"
