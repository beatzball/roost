...
1. **Some dead turns still show ✅ done.** A turn that dies *after* the model has started answering has a reply — the part it got out — so it looks finished. So does every turn on a machine with neither `python3` nor `jq`, where roost cannot read the payload and will not guess. If a codex pane goes ✅ suspiciously fast, look at it.
2. **A turn that ends with nothing to say reads 💥 error**, even if nothing went wrong. Nobody has seen codex do this on a healthy turn, but roost cannot tell the two apart, and a false 💥 is the safer mistake: it makes you look.
