...
once, and the opencode plugin registers its hooks once even when opencode loads
it from both places.
...
**Changing the config yourself will switch the badges off.** Codex stores a hash of each hook entry when you trust it and silently skips any entry that no longer matches — a changed command, or even a changed `timeout`. If you edit `hooks.json`, run `codex` and re-trust through `/hooks`. For the same reason, what `roost hooks codex` prints stays fixed across roost releases; roost changes `adapters/codex/roost-codex-hook` instead, which codex re-reads every run.

It has changed once. The `Interrupt` entry now asks for a `timeout` of 3 rather than 10, because 3 is codex's cap for that event: it was clamping the 10 down and printing a warning with your `hooks.json` path in it on every start. The hook ran with three seconds before and runs with three seconds now — only the warning is gone. If your machine was wired before that, run `roost install`. `roost doctor` tells you while the old number is still there.

**Codex may not ask you anything, and that is not a failed install.** On codex 0.154.0 a `timeout` is not part of the hash stored when you trust a hook, so the new number lands under your existing trust entry and nothing is prompted; an appended argument to the command still moves that hash, which is how we know the measurement is real. An older codex does hash the timeout. So if "Hooks need review" appears, answer "Trust all and continue" once, for `Interrupt` alone — and if it does not appear, you are already done.
