...
1. Read `#{pane_pid}` with one `tmux display-message`. It is a new call on
   the busy-transition path only — measured at 5.89, 6.36 and 6.54 ms per call
   (100 calls through a shell against a throwaway server, three rounds, shell
   spawn included, as in §3.5). The hot-path read above the bail is
