...
//
// Fill in the path to your amux checkout; `amux doctor` prints this exact
...
const report = (state) =>
  new Promise((resolve) => {
    try {
      // AMUX_AGENT_NAME tells amux-agent-state's sink what to label an
      // unnamed pane, instead of falling back to @amux-name-default's
      // Claude-flavoured "claude" — every opencode pane would otherwise
      // read "claude" on its border and in the switcher.
      execFile(
        "amux",
        ["state", state],
        { env: { ...process.env, AMUX_AGENT_NAME: "opencode" }, timeout: 5000 },
        () => resolve()
      )
    } catch {
      resolve()
